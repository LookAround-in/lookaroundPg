...
const fetchProperties = async (
  ...
  const response = await fetch(
    `/api/v1/pg/getExplorePg?page=${page}&limit=${limit}`,
    {
      method: "GET",
      headers: {
...
function AdminPropertyList({
  ...
  const {
    data: explorePropertiesData,
    refetch,
    isError,
    isLoading,
    isPending,
    error,
  } = useQuery<ExploreApiResponse>({
  ...
  const { mutate: createPgDeleteMutation, isPending: pendingRequest } =
    useMutation({
  ...
  const stats = {
    ...
    totalViews: properties.reduce((sum, p) => sum + (p?.reviewCount || 0), 0),
  };
  return (
    <>
      {/* Stats Cards */}
        ...
        <div className="flex items-center justify-center py-12">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">
              {(error as Error)?.message || "Failed to load property details"}
            </p>
            ...
            filteredProperties.map((property) => (
              <div key={property.id} className="relative group">
                ...
                <div className="absolute top-2.5 left-24">
                  <Badge
                    variant={
                      property.sharingTypes.length > 0 ? "default" : "secondary"
                    }
                  >
          ...
          <div className="py-4">
            <div className="bg-gray-50 p-4 rounded-lg mb-4">
              ...
              <p className="text-sm text-gray-600">
                Deleting this property will permanently remove all its details,
                listings, and associated data from LookAroundPG. This action
                cannot be undone. Please ensure you want to proceed before
                confirming deletion.
              </p>
            </div>
